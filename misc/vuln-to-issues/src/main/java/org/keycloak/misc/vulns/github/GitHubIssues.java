...
public class GitHubIssues {
    ...
    private static GHIssue createIssue(String ghToken, GHRepository repository, Vuln vuln) throws IOException, URISyntaxException, InterruptedException {
        ...
        issueBuilder.label("status/triage").label("source/scan-dependencies").label("area/dependencies").label("kind/cve").label("severity/" + vuln.severity().toLowerCase());

        getBackportLabels(vuln).forEach(issueBuilder::label);

        GHIssue issue = issueBuilder.create();

        if (repository.getOwner().equals("keycloak")) {
