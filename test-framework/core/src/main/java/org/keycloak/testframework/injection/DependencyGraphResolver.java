...
public class DependencyGraphResolver {
    ...
    private void scan(Dependency dependency) {
        ...
        RequestedInstance matchingInstance = registry.getRequestedInstances().stream().filter(r -> r.getValueType().equals(dependency.getClass()) && Objects.equals(r.getRef(), dependency.ref())).findFirst().orElse(null);
        if (matchingInstance == null) {
            matchingInstance = missingInstances.stream().filter(r -> r.getValueType().equals(dependency.valueType()) && Objects.equals(r.getRef(), dependency.ref())).findFirst().orElse(null);
        }

        if (matchingInstance == null) {
            Supplier<?, ?> supplier = registry.getExtensions().findSupplierByType(dependency.valueType());
            Annotation defaultAnnotation = DefaultAnnotationProxy.proxy(supplier.getAnnotationClass(), dependency.ref() != null ? dependency.ref() : "");
